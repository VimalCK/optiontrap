...
const Holdings: React.FC = () => {
  ...
  // Start/stop WebSocket based on market status
  useEffect(() => {
    ...
    <div>
      ...
      {error && (
        <div className="card holdings-error">
          <p>{error}</p>
          <button className="btn btn--primary" onClick={loadHoldings} style={{ marginTop: 12 }}>
            Retry
          </button>
        </div>
      )}
          ...
          <div className="card" style={{ marginTop: 24 }}>
            <div className="allocation-header">
              <div>
                ...
                <p className="card__description">
                  Portfolio weight by current market value
                </p>
              ...
              <div className="allocation-toggle">
                <button
                  className={`allocation-toggle__btn ${allocationView === 'bar' ? 'active' : ''}`}
                  onClick={() => setAllocationView('bar')}
                  aria-pressed={allocationView === 'bar'}
                  title="Bar view"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M3 3v18h18" /><path d="M7 16h3" /><path d="M7 12h7" /><path d="M7 8h11" />
                  </svg>
                </button>
                <button
                  className={`allocation-toggle__btn ${allocationView === 'treemap' ? 'active' : ''}`}
                  onClick={() => setAllocationView('treemap')}
                  aria-pressed={allocationView === 'treemap'}
                  title="Treemap view"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="allocation-list">
                {allocation.map((item) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={`allocation-item ${isExpanded ? 'allocation-item--expanded' : ''}`}
      ...
      {!loading && !error && holdings.length === 0 && (
        <div className="card">
          ...
          <p className="card__description">
            You don't have any equity holdings in your DEMAT account.
          </p>
        </div>
      )}
    </div>
 ...
 * Attempts to produce rectangles with aspect ratios close to 1
 */
function computeTreemapLayout(
  ...
  if (items.length === 1) {
    results.push({ x, y, w: width, h: height, index: items[0].index });
    return results;
  }
  ...
  return (
    <div className="treemap">
      {rects.map((rect) => {
        ...
        // Color based on P&L: green for profit, red for loss, intensity by magnitude
        const intensity = Math.min(Math.abs(pnlPct) / 20, 1); // cap at 20%
          ...
          >
            {rect.w > 8 && rect.h > 12 && (
              <div className="treemap__cell-content">
                ...
                {rect.w > 12 && rect.h > 20 && (
                  <span className="treemap__cell-pnl" style={{ color: textColor }}>
                    {pnlPct >= 0 ? '+' : ''}{pnlPct.toFixed(1)}%
                  </span>
                )}
                {rect.w > 15 && rect.h > 28 && (
                  <span className="treemap__cell-pct">{item.percentage.toFixed(1)}%</span>
                )}
              </div>
            )}
          </div>
